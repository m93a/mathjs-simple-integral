...
function factory (type, config, load, typed) {
  ...
  const integral = typed('integral', {
    'Node, SymbolNode, Object': function (expr, variable, options) {
      ...
      const context = new IntegrationContext(variable, options)

      // Simplify the integral
      const simplifiedExpr = preprocessIntegrand(expr, context)
      let integralResult = _integral(simplifiedExpr, context)
  ...
  IntegrationContext.prototype.printDebug = function (text) {
    if (this.options.debugPrint) {
      let indent = ''
      for (let i = 0; i < this.debugIndent; i++) {
        indent += '  '
      }
    ...
    function reduceFunctions (expr) {
      ...
      function helper (expr) {
        if (!context.isConstant(expr) && expr.type === 'FunctionNode') {
          const funcName = typeof expr.fn === 'string' ? expr.fn : expr.fn.name
  ...
  integral.rules = [
    ...
    function (expr, context, subIntegral) {
      const simplified = simplify.simplifyCore(expr, context)
    ...
    function (expr, context, subIntegral) {
      if (expr.type === 'ParenthesisNode') {
        const int = subIntegral(expr.content, context, 'parentheses removal')
    ...
    function (expr, context, subIntegral) {
      if (expr.type === 'OperatorNode' && (expr.op === '+' || expr.op === '-')) {
        const childInts = expr.args.map(function (expr) {
    ...
    function (expr, context, subIntegral) {
      if (expr.type === 'OperatorNode' && expr.op === '*') {
        ...
        if (constantFactors.length !== 0) {
          const constantNode = factorsToNode(constantFactors)
          const nonConstantNode = factorsToNode(nonConstantFactors)

          const nonConstantIntegral = subIntegral(nonConstantNode, context, 'multiplication by constant')
    ...
    function (expr, context) {
      if (expr.type === 'OperatorNode' && expr.op === '^' && expr.args[0].equals(context.variable) && context.isConstant(expr.args[1])) {
        ...
        const exponentValue = getNumericValue(expr.args[1])
        if (exponentValue === -1) {
    ...
    function (expr, context, subIntegral) {
      if (expr.type === 'OperatorNode' && expr.op === '*') {
        const argsAsPower = expr.args.map(getExprInPowerForm)

        // Collect common bases (this is O(n^2) worst case)
        const reducedArgs = argsAsPower.reduce(function (acc, exprPower) {
        ...
        if (reducedArgs.length < expr.args.length) {
          ...
          const reducedExpr = powerFactorsToNode(reducedArgs)
    ...
    function (expr, context, subIntegral) {
      if (expr.type === 'OperatorNode' && expr.op === '*') {
        ...
        if (wasChange) {
          const int = subIntegral(new OperatorNode('*', 'multiply', processedArgs), context, 'removing unary +/- from factors')
          if (int) {
    ...
    function (expr, context, subIntegral) {
      if (expr.type === 'OperatorNode' && expr.op === '*') {
        ...
        if (sumNode !== null) {
          const newTerms = sumNode.args.map(function (term) {
    ...
    function (expr, context, subIntegral) {
      let createIntegralWrapper = null

      const uniqueParent = getParentOfUniqueVariable(expr)
      if (uniqueParent !== null && uniqueParent.type === 'OperatorNode') {
        ...
        if (createIntegralWrapper !== null) {
          const preIntegral = replaceNodeInTree(expr, uniqueParent, context.variable.clone())
          const int = subIntegral(preIntegral, context, 'linear substitution')
          if (int) {
            const backSubstituted = replaceNodeInTree(int, context.variable, uniqueParent)
    ...
    function (expr, context, subIntegral) {
      const MaxExponentExpanded = 10

      if (expr.type === 'OperatorNode' && expr.op === '^') {
        const multipliedOut = tryMultiplyOut(expr)
        if (multipliedOut) {
          const int = subIntegral(multipliedOut, context, 'reducing power')
          if (int) {
            return int
          }
        }
        ...
        for (let i = 0; i < expr.args.length; i++) {
          const multipliedOutChild = tryMultiplyOut(expr.args[i])
          if (multipliedOutChild) {
            const int = subIntegral(new OperatorNode('*', 'multiply', multipliedOutChild.args.concat(
              expr.args.slice(0, i),
              expr.args.slice(i + 1)
      ...
      function tryMultiplyOut (expr) {
        if (expr.type === 'OperatorNode' && expr.op === '^' && !context.isConstant(expr.args[0])) {
          ...
          if (Number.isInteger(exponentValue) && exponentValue > 1 && exponentValue <= MaxExponentExpanded) {
            const multiplicator = expr.args[0]

            const multiplicand =
              exponentValue === 2
                ? expr.args[0]
                : new OperatorNode('^', 'pow', [expr.args[0], new ConstantNode(exponentValue - 1)])

            return new OperatorNode('*', 'multiply', [multiplicator, multiplicand])
          }
        }
  ...
  function _integral (expr, context, lastRuleComment) {
    ...
    let debugComment = lastRuleComment ? lastRuleComment + ': ' : ''
    ...
    for (let i = 0; i < context.rules.length; i++) {
      const result = context.rules[i](expr, context, _integral)
