...
function factory(type, config, load, typed) {
  ...
  let integral = typed('integral', {
    'Node, SymbolNode, Object': function(expr, variable, options) {
      ...
      let context = new IntegrationContext(variable, options);

      // Simplify the integral
      let simplifiedExpr = preprocessIntegrand(expr, context);
    ...
    function reduceFunctions(expr) {
      ...
      function helper(expr) {
        if (!context.isConstant(expr) && expr.type === "FunctionNode") {
          let funcName = typeof expr.fn === "string" ? expr.fn : expr.fn.name;
  ...
  integral.rules = [
    ...
    function(expr, context, subIntegral) {
      let simplified = simplify.simplifyCore(expr, context);
    ...
    function(expr, context, subIntegral) {
      if (expr.type === 'ParenthesisNode') {
        let int = subIntegral(expr.content, context, "parentheses removal");
    ...
    function(expr, context, subIntegral) {
      if (expr.type === "OperatorNode" && (expr.op === '+' || expr.op === '-')) {
        let childInts = expr.args.map(function(expr) {
    ...
    function(expr, context, subIntegral) {
      if (expr.type === "OperatorNode" && expr.op === '*') {
        ...
        if (constantFactors.length !== 0) {
          let constantNode = factorsToNode(constantFactors);
          let nonConstantNode = factorsToNode(nonConstantFactors);

          let nonConstantIntegral = subIntegral(nonConstantNode, context, "multiplication by constant");
    ...
    function(expr, context) {
      if (expr.type === "OperatorNode" && expr.op === '^' && expr.args[0].equals(context.variable) && context.isConstant(expr.args[1])) {
        ...
        let exponentValue = getNumericValue(expr.args[1]);
        if (exponentValue === -1) {
    ...
    function(expr, context, subIntegral) {
      if (expr.type === "OperatorNode" && expr.op === '*') {
        let argsAsPower = expr.args.map(getExprInPowerForm);

        // Collect common bases (this is O(n^2) worst case)
        let reducedArgs = argsAsPower.reduce(function(acc, exprPower) {
        ...
        if (reducedArgs.length < expr.args.length) {
          ...
          let reducedExpr = powerFactorsToNode(reducedArgs);
    ...
    function(expr, context, subIntegral) {
      if (expr.type === "OperatorNode" && expr.op === '*') {
        ...
        if (wasChange) {
          let int = subIntegral(new OperatorNode('*', 'multiply', processedArgs), context, "removing unary +/- from factors");
          if (int) {
    ...
    function(expr, context, subIntegral) {
      if (expr.type === "OperatorNode" && expr.op === '*') {
        ...
        if (sumNode !== null) {
          let newTerms = sumNode.args.map(function(term) {
    ...
    function(expr, context, subIntegral) {
      ...
      let uniqueParent = getParentOfUniqueVariable(expr);
      if (uniqueParent !== null && uniqueParent.type === "OperatorNode") {
        ...
        if (createIntegralWrapper !== null) {
          let preIntegral = replaceNodeInTree(expr, uniqueParent, context.variable.clone());
          let int = subIntegral(preIntegral, context, "linear substitution");
          if (int) {
            let backSubstituted = replaceNodeInTree(int, context.variable, uniqueParent);
    ...
    function(expr, context, subIntegral) {
      let MaxExponentExpanded = 10;

      if (expr.type === 'OperatorNode' && expr.op === '^') {
        let multipliedOut = tryMultiplyOut(expr);
        if (multipliedOut) {
          let int = subIntegral(multipliedOut, context, "reducing power");
          if (int) {
            return int;
          }
        }
        ...
        for(let i = 0; i < expr.args.length; i++) {
          let multipliedOutChild = tryMultiplyOut(expr.args[i]);
          if (multipliedOutChild) {
            let int = subIntegral(new OperatorNode('*', 'multiply', multipliedOutChild.args.concat(
              expr.args.slice(0, i),
              expr.args.slice(i+1)
  ...
  function _integral(expr, context, lastRuleComment) {
    ...
    for(let i = 0; i < context.rules.length; i++) {
      let result = context.rules[i](expr, context, _integral);
